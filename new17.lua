...
function Library:CreateWindow(config)
	...
	Create("TextLabel", {
		Parent = titleBar,
		Position = UDim2.new(0, 48, 0, 10), -- Adjusted for shorter title bar
	...
	if Window.Description ~= "" then
		Create("TextLabel", {
			Parent = titleBar,
			Position = UDim2.new(0, 48, 0, 26), -- Adjusted for shorter title bar
	...
	local buttonHolder = Create("Frame", {
		...
		Position = UDim2.new(1, -10, 0, 8), -- Adjusted for shorter title bar
	...
	if Window.CanClose then
		createButton("rbxassetid://114783304987099", UDim2.new(0, 16, 0, 20), UDim2.new(0, 44, 0, 2), function() -- More space
			Window:Destroy()
		end)
	end
	...
	local tabContainer = Create("Frame", {
		...
		Size = UDim2.new(0, 150, 0, 324), -- LONGER (was 135)
		BackgroundTransparency = 1
	})
	...
	local contentScroll = Create("ScrollingFrame", {
		Parent = main,
		Position = UDim2.new(0.265, 0, 0.116, 0), -- Adjusted for longer tab section
		Size = UDim2.new(0, 455, 0, 398), -- Adjusted size
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	function Window:CreateTab(config)
		...
		local btn = Create("TextButton", {
			...
			TextSize = isFirst and 15 or 14, -- BIGGER when selected
			TextXAlignment = Enum.TextXAlignment.Left,
		...
		if icon and Icons[icon] then
			...
			btn.Text = "   " .. name -- Add spacing for icon
		end

		Create("UIPadding", {Parent = btn, PaddingLeft = UDim.new(0, 10)})

		-- Static underline (removed animation)
		...
		local layout = Create("UIListLayout", {Parent = content, Padding = UDim.new(0, 10)}) -- EQUAL SPACING
		...
		table.insert(Window.Connections, btn.MouseButton1Click:Connect(function()
			...
			for _, tab in ipairs(Window.Tabs) do
				...
				if tab.IconLabel then
					Tween(tab.IconLabel, {ImageColor3 = Color3.fromRGB(150, 150, 150)}):Play()
				end
			...
			if iconLabel then
				Tween(iconLabel, {ImageColor3 = Color3.fromRGB(255, 255, 255)}):Play()
			end
		...
		function Tab:Render()
			...
			for _, elem in ipairs(Tab.Elements) do
				...
				elseif elem.Type == "Button" then
					-- TALLER button (50 instead of 42)
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side == 1 and 0.5 or 1, elem.Side == 1 and -4 or 0, 0, 50),
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0,
					...
					-- Text like Paragraph (centered properly)
				...
				elseif elem.Type == "Slider" then
					...
					local valueLabel = Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(1, 0.5),
						Position = UDim2.new(0.974, -60, 0.5, 0), -- LEFT of slider
				...
				elseif elem.Type == "Dropdown" then
					...
					-- FIX: Dropdown menu parented to GUI
					local dropdownMenu = Create("ScrollingFrame", {
						Parent = gui, -- Parent to GUI, not frame
						Position = UDim2.new(0, 0, 0, 0), -- Will be positioned correctly
					...
					table.insert(Window.Connections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
						if elem.Open and (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
				...
				elseif elem.Type == "Colorpicker" then
					...
					colorPreview.MouseButton1Click:Connect(function()
						-- FIX: Create proper color picker dialog
						...
						Create("UICorner", {CornerRadius = UDim.new(0.05, 0), Parent = preview})
						...
						for i, color in ipairs(colors) do
							local colorBtn = Create("TextButton", {
								...
								Position = UDim2.new(0, 100 + ((i-1) % 4) * 45, 0, 40 + math.floor((i-1) / 4) * 45),
						...
						Create("UICorner", {CornerRadius = UDim.new(0.1, 0), Parent: doneBtn})
						doneBtn.MouseButton1Click:Connect(function() dialog:Destroy() end)
					end)
