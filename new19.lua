-- Fixed & Enhanced Roblox UI Library (Fluent-Style)
...
-- Load Lucide Icons (fixed URL and loading)
local Icons = {}
...
if success then
	...
	if loadFunc then
		...
		warn("Failed to load Lucide Icons:", err)
	end
else
	warn("Failed to fetch Lucide Icons library:", response)
	Icons = {}
end

-- Global keybind handler (fixed duplicate registration)
local keybindConnections = {}
table.insert(keybindConnections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
table.insert(keybindConnections, UserInputService.InputEnded:Connect(function(input)
	for _, keybind in pairs(Library.Keybinds) do
...
function Library:GetThemeColor(key)
	local theme = Themes[Library.Config.Theme] or Themes.Dark
	return theme[key] or Themes.Dark[key]
end
...
function Library:CreateWindow(config)
	...
	local main = Create("Frame", {
		...
		BackgroundColor3 = self:GetThemeColor("Main"),
		BorderSizePixel = 0,
		ClipsDescendants = true
	})

	Create("UICorner", {CornerRadius = UDim.new(0.02, 0), Parent = main})
	Create("UIStroke", {Color = self:GetThemeColor("Stroke"), Thickness = 1, Parent = main})
	...
	Create("TextLabel", {
		...
		Text = Window.Title,
		TextColor3 = self:GetThemeColor("Text"),
		TextSize = 14,
	...
	if Window.Description ~= "" then
		Create("TextLabel", {
			...
			TextColor3 = self:GetThemeColor("TextDim"),
			TextSize = 11,
			TextXAlignment = Enum.TextXAlignment.Left
		})
	end
	...
	local contentBg = Create("Frame", {
		...
		BackgroundColor3 = self:GetThemeColor("Content"),
		BackgroundTransparency = 0.3,
		BorderSizePixel = 0
	})

	Create("UICorner", {CornerRadius = UDim.new(0.01, 0), Parent = contentBg})
	Create("UIStroke", {Color = self:GetThemeColor("Stroke"), Thickness = 1, Parent = contentBg})
	...
	if not content or not isReady then
		content = "rbxassetid://116523599334562" -- Fallback
	end
	...
	function Window:CreateTab(config)
		...
		-- Tab button with icon support (fixed iconLabel scope)
		local btn = Create("TextButton", {
			...
			Font = Enum.Font.Gotham,
			Text = name,
			TextColor3 = isFirst and self:GetThemeColor("Text") or self:GetThemeColor("TextDim"),
			TextSize = 14,
			TextXAlignment = Enum.TextXAlignment.Left,
		...
		if icon and Icons[icon] then
			iconLabel = Create("ImageLabel", {
				...
				ScaleType = Enum.ScaleType.Fit,
				ImageColor3 = isFirst and self:GetThemeColor("Text") or self:GetThemeColor("TextDim")
			})
			btn.Text = "   " .. name
		end
		...
		local underline = Create("Frame", {
			...
			BackgroundColor3 = self:GetThemeColor("Accent"),
			BorderSizePixel = 0,
		...
		table.insert(Window.Connections, btn.MouseButton1Click:Connect(function()
			...
			for _, tab in ipairs(Window.Tabs) do
				Tween(tab.Button, {TextColor3 = self:GetThemeColor("TextDim")}):Play()
				Tween(tab.Underline, {BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 2)}):Play()
				if tab.IconLabel then
					Tween(tab.IconLabel, {ImageColor3 = self:GetThemeColor("TextDim")}):Play()
				end
				tab.Content.Visible = false
			end
			Tween(btn, {TextColor3 = self:GetThemeColor("Text")}):Play()
			Tween(underline, {BackgroundTransparency = 0, Size = UDim2.new(1, 0, 0, 2)}):Play()
			if iconLabel then
				Tween(iconLabel, {ImageColor3 = self:GetThemeColor("Text")}):Play()
			end
			...
			for _, elem in ipairs(Tab.Elements) do
				...
				if elem.Type == "Paragraph" then
					...
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(1, 0, 0, 0),
						BackgroundColor3 = self:GetThemeColor("Element"),
						BorderSizePixel = 0,
						AutomaticSize = Enum.AutomaticSize.Y
					})
					Create("UICorner", {CornerRadius = UDim.new(0.05, 0), Parent = frame})
					Create("UIStroke", {Color = self:GetThemeColor("Stroke"), Thickness = 1, Parent = frame})
				...
				elseif elem.Type == "Button" then
					...
					btn.MouseButton1Click:Connect(function()
						pcall(function()
							...
							task.delay(0.1, function() frame.BackgroundColor3 = self:GetThemeColor("Element") end)
							elem.Callback()
						end)
					end)
				...
				elseif elem.Type == "Toggle" then
					...
					local toggleFrame = Create("Frame", {
						...
						BackgroundColor3 = elem.Value and self:GetThemeColor("Accent") or Color3.fromRGB(30, 30, 30),
						BorderSizePixel = 0
					})
					Create("UICorner", {CornerRadius = UDim.new(0.5, 0), Parent = toggleFrame})
					Create("UIStroke", {Color = self:GetThemeColor("Stroke"), Thickness = 1, Parent = toggleFrame})
					...
					btn.MouseButton1Click:Connect(function()
						pcall(function()
							...
							Tween(toggleFrame, {BackgroundColor3 = elem.Value and self:GetThemeColor("Accent") or Color3.fromRGB(30, 30, 30)}, 0.2):Play()
				...
				elseif elem.Type == "Slider" then
					...
					-- Value label on left of slider
					local valueLabel = Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(1, 0.5),
						Position = UDim2.new(0.974, -55, 0.5, 0),
				...
				elseif elem.Type == "Dropdown" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side == 1 and 0.5 or 1, elem.Side == 1 and -4 or 0, 0, elem.Open and math.min(#elem.Values * 26 + 48, 198) or 42),
						BackgroundColor3 = self:GetThemeColor("Element"),
						BorderSizePixel = 0,
					...
					local dropdownMenu = Create("ScrollingFrame", {
						...
						Size = UDim2.new(0.95, 0, 0, math.min(#elem.Values * 26, 150)),
						BackgroundColor3 = Color3.fromRGB(20, 20, 20),
						BorderSizePixel = 0,
						ScrollBarThickness = 3,
						Visible = elem.Open,
						ZIndex = 10,
						CanvasSize = UDim2.new(0, 0, 0, #elem.Values * 26),
						ScrollingDirection = Enum.ScrollingDirection.Y
					})
				...
				elseif elem.Type == "Colorpicker" then
					...
					if elem.Transparency then
						colorPreview.BackgroundTransparency = elem.TransValue or 0
					end
					...
					if elem.Open then
						-- Placeholder for expanded color picker (simplified)
						local colorHint = Create("TextLabel", {
							Parent = frame,
							Position = UDim2.new(0.026, 0, 0, 42),
							Size = UDim2.new(0.6, 0, 0, 18),
							BackgroundTransparency = 1,
							Font = Enum.Font.Gotham,
							Text = "Click to pick color (WIP)",
							TextColor3 = self:GetThemeColor("TextDim"),
							TextSize = 11,
							TextXAlignment = Enum.TextXAlignment.Left
						})
	...
	function Window:Destroy()
		pcall(function()
			...
			-- Remove keybinds from global list
			for _, tab in ipairs(Window.Tabs) do
				for _, elem in ipairs(tab.Elements) do
					if elem.Type == "Keybind" then
						for i, keybind in ipairs(Library.Keybinds) do
							if keybind == elem then
...
-- Save Manager (Fixed with exploit checks)
Library.SaveManager = {
	Library = nil,
...
local function hasExploitFunctions()
...
function Library.SaveManager:Save(name)
	...
	if not hasExploitFunctions() then return warn("Exploit functions not available") end
	...
	writefile(self.Folder .. "/" .. name .. ".json", HttpService:JSONEncode(config))
	self:Refresh()
end
...
function Library.SaveManager:Refresh()
	self.Configs = {}
	if hasExploitFunctions() and isfolder(self.Folder) then
...
function Library.SaveManager:Delete(name)
	if not name or name == "" then return end
	if not hasExploitFunctions() then return end
...
function Library.InterfaceManager:Save()
	...
	if hasExploitFunctions() and isfolder(self.Folder) then
		writefile(self.Folder .. "/interface.json", HttpService:JSONEncode(config))
	end
end
...
function Library.InterfaceManager:Load()
	...
	if isfile(path) then
		...
		if success then
			...
			Library.Config.Theme = self.Theme
		end
	end
end
...
function Library.InterfaceManager:BuildInterfaceSection(tab)
	tab:AddDropdown({
		...
		Callback = function(value)
			self.Theme = value
			Library.Config.Theme = value
			self:Save()
			-- Re-render all windows for theme change
