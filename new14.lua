-- Enhanced Roblox UI Library (Fully Fixed v3.1)
...
function Library:CreateWindow(config)
	...
	local titleBar = Create("Frame", {
		Name = "TitleBar",
		Parent = main,
		Size = UDim2.new(1, 0, 0, 40), -- Shorter title bar
		BackgroundTransparency = 1
	})
	...
	if Window.CanClose then
		createButton("rbxassetid://114783304987099", UDim2.new(0, 16, 0, 20), UDim2.new(0, 44, 0, 2), function()
			Window:Destroy()
		end)
	end
	...
	local tabContainer = Create("Frame", {
		...
		Size = UDim2.new(0, 160, 0, 324), -- Wider tabs
		BackgroundTransparency = 1
	})
	...
	local contentScroll = Create("ScrollingFrame", {
		Parent = main,
		Position = UDim2.new(0.247, 0, 0.116, 0), -- Adjusted for wider tabs
		Size = UDim2.new(0.753, 0, 0.883, 0),
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	function Window:CreateTab(config)
		...
		local btn = Create("TextButton", {
			...
			TextSize = isFirst and 15 or 14, -- Bigger text when active
			TextXAlignment = Enum.TextXAlignment.Left,
		...
		if icon and Icons[icon] then
			iconLabel = Create("ImageLabel", {
				...
				AnchorPoint = Vector2.new(0, 0.5),
				Size = UDim2.new(0, isFirst and 20 or 18, 0, isFirst and 20 or 18), -- Bigger icon when active
				BackgroundTransparency = 1,
		...
		table.insert(Window.Connections, btn.MouseButton1Click:Connect(function()
			...
			for _, tab in ipairs(Window.Tabs) do
				Tween(tab.Button, {TextColor3 = Color3.fromRGB(150, 150, 150), TextSize = 14}):Play()
			...
			Tween(btn, {TextColor3 = Color3.fromRGB(255, 255, 255), TextSize = 15}):Play()
		...
		function Tab:Render()
			...
			for _, elem in ipairs(Tab.Elements) do
				...
				elseif elem.Type == "Button" then
					-- Taller button with centered text
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side == 1 and 0.5 or 1, elem.Side == 1 and -4 or 0, 0, 48), -- Taller
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0,
					...
					-- Title with proper vertical centering
					Create("TextLabel", {
						Parent = frame,
						Position = UDim2.new(0.026, 0, 0.5, 0), -- Centered vertically
						AnchorPoint = Vector2.new(0, 0.5),
					...
					if elem.Description ~= "" then
						Create("TextLabel", {
							Parent = frame,
							Position = UDim2.new(0.026, 0, 0.5, 10), -- Below title, centered
							AnchorPoint = Vector2.new(0, 0.5),
				...
				elseif elem.Type == "Input" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(1, 0, 0, 48), -- Taller
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0
					})
					Create("UICorner", {CornerRadius = UDim.new(0.1, 0), Parent = frame})
				...
				elseif elem.Type == "Slider" then
					...
					local valueLabel = Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(1, 0.5),
						Position = UDim2.new(0.974, -55, 0.5, 0), -- Left of slider
				...
				elseif elem.Type == "Dropdown" then
					...
					-- Dropdown menu (fixed to be overlay)
					local dropdownMenu = Create("ScrollingFrame", {
						Parent = gui, -- Parent to GUI, not frame
						Position = UDim2.new(0, 0, 0, 0), -- Will be set properly
						Size = UDim2.new(0, 200, 0, math.min(#elem.Values * 26, 150)),
						BackgroundColor3 = Color3.fromRGB(20, 20, 20),
						BorderSizePixel = 0,
					...
					table.insert(Library.OpenFrames, dropdownMenu)
					...
					dropdownBtn.MouseButton1Click:Connect(function()
						if elem.Open then
							...
							dropdownMenu.Size = UDim2.new(0, absSize.X, 0, math.min(#elem.Values * 26, 150))
					...
					table.insert(Window.Connections, UserInputService.InputBegan:Connect(function(input, gameProcessed)
						if input.UserInputType == Enum.UserInputType.MouseButton1 and elem.Open then
							...
							if not (mousePos.X >= menuPos.X and mousePos.X <= menuPos.X + menuSize.X and
							   ...
							   not (mousePos.X >= absPos.X and mousePos.X <= absPos.X + absSize.X and
				...
				elseif elem.Type == "Colorpicker" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side == 1 and 0.5 or 1, elem.Side == 1 and -4 or 0, 0, 48), -- Consistent height
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0
					})
					Create("UICorner", {CornerRadius = UDim.new(0.1, 0), Parent = frame})
					...
					colorPreview.MouseButton1Click:Connect(function()
						...
						-- HSV color map
						local hue, sat, val = Color3.toHSV(elem.Value)
						...
						local hueCursor = Create("Frame", {
							...
							Position = UDim2.new(0, 0, 0, hue * 160),
							BackgroundColor3 = Color3.fromRGB(255, 255, 255),
							BorderSizePixel = 0,
							ZIndex = 1001
						})
						...
						if elem.Transparency then
							preview.BackgroundTransparency = elem.TransValue
						end
						...
						doneBtn.MouseButton1Click:Connect(function()
							elem.Value = preview.BackgroundColor3
							elem.TransValue = preview.BackgroundTransparency
							elem.Callback(elem.Value)
							Tab:Render()
							dialog:Destroy()
						end)
						...
						-- Color map input
						table.insert(Window.Connections, colorMap.InputBegan:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.MouseButton1 then
								local conn
								conn = RunService.RenderStepped:Connect(function()
									...
									local localPos = colorMap:GetRelativeMousePosition()
									sat = math.clamp(localPos.X / colorMap.AbsoluteSize.X, 0, 1)
									val = 1 - math.clamp(localPos.Y / colorMap.AbsoluteSize.Y, 0, 1)
						...
						-- Hue slider input
						table.insert(Window.Connections, hueSlider.InputBegan:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.MouseButton1 then
								local conn
								conn = RunService.RenderStepped:Connect(function()
									if not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
									...
									local localPos = hueSlider:GetRelativeMousePosition()
									hue = math.clamp(localPos.Y / hueSlider.AbsoluteSize.Y, 0, 1)
									hueCursor.Position = UDim2.new(0, 0, 0, hue * 160)
						...
						if elem.Transparency then
							local transSlider = Create("Frame", {
								Parent = dialog,
							...
							local transFill = Create("Frame", {
								Parent = transSlider,
								Size = UDim2.new(1 - (elem.TransValue or 0), 0, 1, 0),
								BackgroundColor3 = Color3.fromRGB(100, 150, 255),
								ZIndex = 1001
							}, {
								Create("UICorner", {CornerRadius = UDim.new(0.5, 0)})
							...
							table.insert(Window.Connections, transSlider.InputBegan:Connect(function(input)
								if input.UserInputType == Enum.UserInputType.MouseButton1 then
									local conn
									conn = RunService.RenderStepped:Connect(function()
										if not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
										...
										local localPos = transSlider:GetRelativeMousePosition()
										elem.TransValue = 1 - math.clamp(localPos.X / transSlider.AbsoluteSize.X, 0, 1)
										transFill.Size = UDim2.new(1 - elem.TransValue, 0, 1, 0)
										preview.BackgroundTransparency = elem.TransValue
									end)
								end
							end)
