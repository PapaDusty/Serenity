...
function Library:CreateWindow(config)
	...
	if Window.CanMinimize then
		createButton("rbxassetid://104255241083015", UDim2.new(0, 14, 0, 14), UDim2.new(0, 20, 0, 4), function()
	...
	if Window.CanClose then
		createButton("rbxassetid://114783304987099", UDim2.new(0, 14, 0, 14), UDim2.new(0, 38, 0, 4), function()
			Window:Destroy()
		end)
	end
	...
	function Window:CreateTab(name)
		...
		local layout = Create("UIListLayout", {Parent = content, Padding = UDim.new(0, 6)})
		...
		function Tab:AddKeybind(config)
			...
			local elem = {
				...
				Active = false,
				Holding = false,
				Listening = false -- ADDED: Initialize listening state
			}
			table.insert(Tab.Elements, elem)
			Tab:Render()
			...
			return {
				...
				Hide = function() elem.Visible = false Tab:Render() end,
				Show = function() elem.Visible = true Tab:Render() end,
				OnClick = function(_, cb) elem.Callback = cb end, -- FIXED: Changed from OnClick to match actual usage
		...
		function Tab:Render()
			-- Clear existing elements but keep the layout
			...
			for _, elem in ipairs(Tab.Elements) do
				...
				elseif elem.Type == "Button" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side > 0 and 0.5 or 1, elem.Side > 0 and -3 or 0, 0, 42),
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0,
				...
				elseif elem.Type == "Input" then
					...
					local inputFrame = Create("Frame", {
						...
						Size = UDim2.new(0.45, 0, 0, 28), -- MADE SKINNIER: reduced from 0.55 to 0.45
				...
				elseif elem.Type == "Slider" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side > 0 and 0.5 or 1, elem.Side > 0 and -3 or 0, 0, 42), -- REDUCED HEIGHT from 50 to 42
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0
					})
					Create("UICorner", {CornerRadius = UDim.new(0.1, 0), Parent = frame})
					...
					-- Slider track - positioned on right side
					local sliderTrack = Create("Frame", {
						Parent = frame,
						AnchorPoint = Vector2.new(1, 0.5),
						Position = UDim2.new(0.974, 0, 0.5, 8),
						Size = UDim2.new(0.45, 0, 0, 6), -- MADE SLIMMER and on right
						BackgroundColor3 = Color3.fromRGB(18, 18, 18),
						BorderSizePixel = 0
					})
				...
				elseif elem.Type == "Dropdown" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side > 0 and 0.5 or 1, elem.Side > 0 and -3 or 0, 0, elem.Open and math.min(#elem.Values * 30 + 50, 200) or 42),
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0
					})
					Create("UICorner", {CornerRadius = UDim.new(0.1, 0), Parent = frame})
					...
					-- Dropdown button on right (like Input)
					local dropdownBtn = Create("TextButton", {
						Parent = frame,
						AnchorPoint = Vector2.new(1, 0.5),
						Position = UDim2.new(0.974, 0, 0.5, 0),
						Size = UDim2.new(0.45, 0, 0, 28), -- SKINNIER like Input
						BackgroundColor3 = Color3.fromRGB(18, 18, 18),
						BorderSizePixel = 0,
						Font = Enum.Font.Gotham,
						Text = elem.Multi and (next(elem.Value) and table.concat(self:GetSelectedOptions(elem.Value), ", ") or "None") or elem.Value,
						TextColor3 = Color3.fromRGB(255, 255, 255),
						TextSize = 13,
						ZIndex = 2
					})
					...
					for _, optionBtn in ipairs(dropdownMenu:GetChildren()) do
					...
					for _, option in ipairs(elem.Values) do
						...
						-- Highlight selected items in multi-dropdown
						if elem.Multi and elem.Value[option] then
						...
						optionBtn.MouseButton1Click:Connect(function()
							if elem.Multi then
								...
								dropdownBtn.Text = next(elem.Value) and table.concat(self:GetSelectedOptions(elem.Value), ", ") or "None"
							else
								elem.Value = option
								elem.Open = false
				...
				elseif elem.Type == "Colorpicker" then
					local frame = Create("Frame", {
						Parent = container,
						Size = UDim2.new(elem.Side > 0 and 0.5 or 1, elem.Side > 0 and -3 or 0, 0, elem.Open and (elem.Transparency and 120 or 100) or 42),
						BackgroundColor3 = Color3.fromRGB(25, 25, 25),
						BorderSizePixel = 0
					})
					Create("UICorner", {CornerRadius = UDim.new(0.1, 0), Parent = frame})
		...
		-- Helper function to get selected options for multi-dropdown
