-- Enhanced Roblox UI Library (Fully Fixed)
...
function Library:CreateWindow(config)
	...
	function Window:CreateTab(name)
		...
		local layout = Create("UIListLayout", {Parent = content, Padding = UDim.new(0, 5)})
		...
		function Tab:AddInput(config)
			...
			local elem = {
				...
				Side = 0 -- Input is always full width
			}
			table.insert(Tab.Elements, elem)
			Tab:Render()
			return {
		...
		function Tab:AddKeybind(config)
			...
			local elem = {
				...
				Active = false,
				Holding = false,
				Side = config.Side or 0 -- FIX: Added missing Side property
			}
			table.insert(Tab.Elements, elem)
			Tab:Render()
		...
		function Tab:AddDivider(config)
			...
			local elem = {
				Type = "Divider",
				Visible = true,
				Side = 0 -- FIX: Added Side property
			}
			table.insert(Tab.Elements, elem)
			Tab:Render()
			return {
			...
			for _, elem in ipairs(Tab.Elements) do
				if elem.Visible then
					...
					if elem.Type == "Paragraph" then
						...
						local textContainer = Create("Frame", {Parent = frame, Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y})
						Create("UIListLayout", {Parent = textContainer, Padding = UDim.new(0, 4)})
					...
					elseif elem.Type == "Button" then
						...
						Create("UIPadding", {Parent = textContainer, PaddingLeft = UDim.new(0, 12)})
						
						Create("TextLabel", {
							Parent = textContainer,
							AnchorPoint = Vector2.new(0, 0.5),
							Position = UDim2.new(0, 0, 0.5, elem.Description ~= "" and -6 or 0),
							Size = UDim2.new(1, -12, elem.Description ~= "" and 0.5 or 1, 0),
							BackgroundTransparency = 1,
							Font = Enum.Font.Gotham,
							Text = elem.Title,
							TextColor3 = Color3.fromRGB(255, 255, 255),
							TextSize = 14,
							TextXAlignment = Enum.TextXAlignment.Left,
							TextYAlignment = elem.Description ~= "" and Enum.TextYAlignment.Bottom or Enum.TextYAlignment.Center
						})
