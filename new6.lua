	...
	for _, keybind in pairs(Library.Keybinds) do
		if keybind.Value == input.KeyCode or keybind.Value == input.UserInputType then
			if keybind.Mode == "Toggle" then
				...
				pcall(keybind.OnClick) -- Support OnClick callback
...
function Library:CreateWindow(config)
	...
	-- Control buttons (smaller close button)
	...
	if Window.CanClose then
		local closeBtn = Create("ImageButton", {
			...
			Size = UDim2.new(0, 18, 0, 18), -- Same size as minimize
			BackgroundTransparency = 1,
	...
	function Window:CreateTab(name)
		...
		local layout = Create("UIListLayout", {Parent = content, Padding = UDim.new(0, 6)}) -- Reduced from 8
		...
		function Tab:Render()
			...
			for _, elem in ipairs(Tab.Elements) do
				...
				if elem.Type == "Paragraph" then
					...
					Tab.CurrentSide = 0 -- Reset after full-width element
				...
				elseif elem.Type == "Button" then
					...
					-- Single text label like toggle (centered)
					local text = elem.Description ~= "" and (elem.Title .. "\n" .. elem.Description) or elem.Title
					Create("TextLabel", {
						Parent = frame,
						AnchorPoint = elem.Description ~= "" and Vector2.new(0, 0) or Vector2.new(0, 0.5),
						Position = elem.Description ~= "" and UDim2.new(0.03, 0, 0, 8) or UDim2.new(0.03, 0, 0.5, 0),
						Size = UDim2.new(0.95, 0, elem.Description ~= "" and 0.5 or 1, 0),
						BackgroundTransparency = 1,
						Font = Enum.Font.Gotham,
						Text = text,
						TextColor3 = elem.Description ~= "" and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(255, 255, 255),
						TextSize = elem.Description ~= "" and 14 or 14,
						TextXAlignment = Enum.TextXAlignment.Left,
						TextYAlignment = elem.Description ~= "" and Enum.TextYAlignment.Top or Enum.TextYAlignment.Center
					})
					
					if elem.Description ~= "" then
						Create("TextLabel", {
							Parent = frame,
							Position = UDim2.new(0.03, 0, 0, 24),
				...
				elseif elem.Type == "Toggle" then
					...
					Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(0, 0.5),
						Position = UDim2.new(0.03, 0, 0.5, 0), -- More spacing
				...
				elseif elem.Type == "Slider" then
					...
					Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(0, 0.5),
						Position = UDim2.new(0.03, 0, 0.5, -10), -- Adjusted for vertical center
					...
					local valueLabel = Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(1, 0.5),
						Position = UDim2.new(0.975, 0, 0.5, -10),
				...
				elseif elem.Type == "Dropdown" then
					...
					Create("TextLabel", {
						Parent = frame,
						AnchorPoint = Vector2.new(0, 0.5),
						Position = UDim2.new(0.03, 0, 0.5, -10),
						Size = UDim2.new(0.6, 0, 0, 18),
						BackgroundTransparency = 1,
						Font = Enum.Font.Gotham,
						Text = elem.Title,
		...
		for _, tab in ipairs(window.Tabs) do
			for _, elem in ipairs(tab.Elements) do
				if elem.Type == "Toggle" then
					config[elem.Title] = {Value = elem.Value, Type = "Toggle"}
				elseif elem.Type == "Input" then
					config[elem.Title] = {Value = elem.Value, Type = "Input"}
				elseif elem.Type == "Slider" then
					config[elem.Title] = {Value = elem.Value, Type = "Slider"}
				elseif elem.Type == "Dropdown" then
					config[elem.Title] = {Value = elem.Value, Type = "Dropdown", Multi = elem.Multi}
				elseif elem.Type == "Colorpicker" then
					config[elem.Title] = {Value = {elem.Value.R, elem.Value.G, elem.Value.B}, Trans = elem.TransValue, Type = "Colorpicker", Transparency = elem.Transparency}
				elseif elem.Type == "Keybind" then
					config[elem.Title] = {Value = elem.Value.Name, Mode = elem.Mode, Type = "Keybind"}
				end
			end
		end
	end
...
function Library.SaveManager:Load(name)
	...
	for _, window in ipairs(self.Library.Windows) do
		for _, tab in ipairs(window.Tabs) do
			for _, elem in ipairs(tab.Elements) do
				if config[elem.Title] and not table.find(self.Ignore, elem.Title) then
					local data = config[elem.Title]
