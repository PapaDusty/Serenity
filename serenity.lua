...
local Library do
	...
	Library = {
		...
		Folders = {
			Directory = "serenity",
			Configs = "serenity/Configs",
			Assets = "serenity/Assets",
		},
		...
		Library.Notification = function(self, Name, Duration, Icon)
			local Items = {} do
				Items["Notification"] = Instances:Create("Frame", {
					...
					AutomaticSize = Enum.AutomaticSize.X,
					BackgroundColor3 = FromRGB(16, 18, 18),
					BackgroundTransparency = 0.2  -- set transparency to 0.2
			...
			Library:Thread(function()
				for Index, Value in Items do
					if Value.Instance:IsA("Frame") then
						Value:Tween(nil, {BackgroundTransparency = 0.2})  -- tween to 0.2 transparency
					elseif Value.Instance:IsA("TextLabel") then
	...
	Library.Window = function(self, Data)
		...
		local Items = {} do
			...
			Items["Side"] = Instances:Create("Frame", {
				...
				BackgroundTransparency = 1,
				BorderColor3 = FromRGB(0, 0, 0),
				Position = UDim2New(0, 0, 0, 0),  -- no top bar, so start at top
				Size = UDim2New(0, 180, 1, 0),     -- full height
				BorderSizePixel = 0,
				BackgroundColor3 = FromRGB(255, 255, 255)
			})

			-- Title frame with stroke and rounded corners (like other elements)
			...
			-- Add stroke and corner to Title frame
			Instances:Create("UIStroke", {
			...
			-- Text with white color (purple gradient removed) and white underline
			Items["RealTitle"] = Instances:Create("TextLabel", {
				Parent = Items["Title"].Instance,
				Name = "\0",
				FontFace = Library.Font,
				TextColor3 = FromRGB(255, 255, 255),   -- plain white
				BorderColor3 = FromRGB(0, 0, 0),
				Text = Window.Name,
				Size = UDim2New(0, 0, 0, 15),
				BackgroundTransparency = 1,
			...
			-- Content area adjusted for new side width
			Items["Content"] = Instances:Create("Frame", {
				Parent = Items["MainFrame"].Instance,
				Name = "\0",
				Position = UDim2New(0, 185, 0, 6),   -- left = 180+5, top = 6 (below buttons)
				BorderColor3 = FromRGB(0, 0, 0),
				Size = UDim2New(1, -191, 1, -12),     -- right padding 6, bottom padding 6
				BorderSizePixel = 0,
				BackgroundColor3 = FromRGB(21, 24, 24)
			...
			-- "Expires in" text – now fully white (transparency removed)
			...
			-- Game name – now just the game name, with semi-transparent white (same grey as old Expires in)
			...
			Items["GameName"] = Instances:Create("TextLabel", {
				...
				FontFace = Library.Font,
				TextColor3 = FromRGB(255, 255, 255),
				TextTransparency = 0.5,   -- now semi‑transparent (the grey the old Expires in had)
				BorderColor3 = FromRGB(0, 0, 0),
				Text = GameName,           -- removed "Game: " prefix
				Size = UDim2New(0, 0, 0, 15),
				BackgroundTransparency = 1,
	...
	Library.Page = function(self, Data)
		...
		local Page = {
			...
			Items = {},
			ColumnsData = {},   -- will hold the column ScrollingFrames
			Active = false
		}
		...
		local Items = {} do
			...
			-- Columns frame (holds the actual column containers)
			Items["Columns"] = Instances:Create("Frame", {
				Parent = Items["Page"].Instance,
				Name = "\0",
				Size = UDim2New(1, -20, 1, -50),  -- leave space for page name and bottom padding
			...
			for Index = 1, 2 do
				local NewColumn = Instances:Create("ScrollingFrame", {
					...
					Size = UDim2New(1, 0, 1, 0),
					BorderSizePixel = 0,
					AutomaticCanvasSize = Enum.AutomaticSize.Y,   -- canvas height matches content
					BackgroundColor3 = FromRGB(255, 255, 255)
				})

				-- Add UIListLayout to stack sections vertically
				Instances:Create("UIListLayout", {
					Parent = NewColumn.Instance,
					Name = "\0",
					Padding = UDimNew(0, 8),          -- spacing between sections
					SortOrder = Enum.SortOrder.LayoutOrder
				})
	...
	-- SubPage functionality has been removed entirely
	...
	Library.Sections.Dropdown = function(self, Data)
		...
		function Dropdown:Set(Option)
			if Dropdown.Multi then
				...
				-- Then activate the ones in Option
				local newValue = {}
				...
				Library.Flags[Dropdown.Flag] = newValue

				local text = #newValue > 0 and TableConcat(newValue, ", ") or "..."
